  ...
  <nav class="fixed bottom-4 left-4 right-4 z-100 safe-area-mb">
    <div class="bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl border border-stone-200/50 mx-auto">
      <div class="flex items-center p-2 min-w-0">
        <BottomNavItem to="/" label="Home" icon-active="heroicons:home-solid" icon-inactive="heroicons:home" />
        <BottomNavItem to="/schedule" label="Calendar" icon-active="heroicons:calendar-days-solid"
          icon-inactive="heroicons:calendar-days" />
        <BottomNavItem to="/articles" label="Articles" icon-active="heroicons:document-text-solid"
          icon-inactive="heroicons:document-text" />
        <BottomNavItem :to="isAuthenticated ? '/account' : '/login'" :label="isAuthenticated ? 'Account' : 'Sign In'"
          icon-active="heroicons:user-solid" icon-inactive="heroicons:user" :auth-routes="['/account', '/login']" />
        <BottomNavItem is-button :on-click="toggleMenu" :is-menu-open="isMenuOpen" is-directory
          :directory-routes="directoryRoutes" label="Directory" icon-active="heroicons:x-mark-solid"
    ...
    <Transition enter-active-class="transition-all duration-300 ease-out"
      ...
      <div v-if="isMenuOpen"
        ...
        <div class="py-2 px-2">
          <RouterLink v-for="item in directoryItems" :key="item.to" :to="item.to" @click="closeMenu"
